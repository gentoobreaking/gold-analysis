...
class ModelEvaluator:
    ...
    def evaluate_classification(
        ...
        pred_dist = {str(k): int(c) for k, c in zip(unique, counts, strict=False)}
        ...
        if feature_df is not None:
            ...
            if not error_features.empty:
                ...
                analysis["feature_analysis"] = {
                    col: float(d) for col, d in diff.items() if not np.isnan(d)
                }
                # 取差異最大的特徵
                sorted_features = sorted(
                    analysis["feature_analysis"].items(), key=lambda x: abs(x[1]), reverse=True
                )[:10]
