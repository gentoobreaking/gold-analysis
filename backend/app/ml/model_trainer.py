...
class ModelTrainer:
    ...
    def load_latest(self, model_name: str | None = None) -> BaseEstimator:
        ...
        # 加載 scaler（若存在）
        scaler_path = (
            self.model_dir / f"{model_info['model_name']}_{model_info['version']}_scaler.pkl"
        )
    ...
    def _get_feature_importance(
        self, model: BaseEstimator, feature_names: list[str] | None = None
    ) -> dict[str, float]:
        """提取特徵重要性"""
        ...
        return {name: float(imp / max_imp) for name, imp in zip(names, importances, strict=False)}
    ...
    def _generate_version(self) -> str:
        ...
        count = sum(1 for k in self.registry._registry["models"] if timestamp in k)
