...
class FeatureEngineer:
    ...
    def fit_transform(self, df: pd.DataFrame, label_col: str = "label") -> pd.DataFrame:
        ...
        self.feature_names = [
            c
            for c in df.columns
            if c not in ["close", "local_sell", "local_buy", "date", label_col]
        ]
        self._fitted = True
    ...
    def _add_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # SMA 排列方向（市場趨勢）
        df["sma_trend"] = (df["sma_short"] > df["sma_medium"]).astype(int) + (
            df["sma_medium"] > df["sma_long"]
        ).astype(int)  # 0=下跌, 1=中性, 2=上漲
    ...
    def _add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bollinger = self._compute_bollinger(
            df["close"], period=cfg.bollinger_period, num_std=cfg.bollinger_std
        )
        ...
        df["bb_position"] = (df["close"] - bollinger["lower"]) / (
    ...
    def _compute_atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
        """計算 ATR（只用收盤價估算，無 high/low/close 時用 close 代替）"""
        close = df["close"]
        tr = abs(close.diff())  # 簡化：只取收盤價差絕對值
    ...
    def _compute_cci(df: pd.DataFrame, period: int = 20) -> pd.Series:
        ...
        mad = close.rolling(window=period).apply(lambda x: np.abs(x - x.mean()).mean())
        return (close - sma) / (0.015 * mad + 1e-10)
    ...
    def _compute_stochastic(
        ...
        highest_high = close.rolling(window=k_period).max()
