...
class FundamentalAnalyzer(GoldAnalysisAgent):
    ...
    async def _analyze_dollar_index(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        if dxy_value > self.dxy_thresholds["strong"]:
            ...
            score = -0.8  # 強美元顯著利空黃金
            reasoning = (
                f"美元指數 DXY={dxy_value:.2f} 處於高位（>{self.dxy_thresholds['strong']}），"
            )
        ...
        return FactorAnalysis(
            ...
            reasoning_en=f"DXY={dxy_value:.2f}. {'Strong USD weighs on gold.' if score < 0 else 'Weak USD supports gold.' if score > 0 else 'Neutral USD impact.'}",  # noqa: E501
    ...
    async def _analyze_real_rate(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        if real_rate < self.real_rate_thresholds["positive"]:
            ...
            score = 0.9  # 低實際利率利好黃金
            reasoning = (
                f"實際利率={real_rate:.2f}% 處於低位（<{self.real_rate_thresholds['positive']}%），"
            )
            ...
            score = -0.9  # 高實際利率利空黃金
            reasoning = (
                f"實際利率={real_rate:.2f}% 處於高位（>{self.real_rate_thresholds['negative']}%），"
            )
        ...
        return FactorAnalysis(
            ...
            confidence=0.90,
            reasoning_zh=reasoning,
            reasoning_en=f"Real rate={real_rate:.2f}%. {'Low real rates support gold.' if score > 0 else 'High real rates weigh on gold.' if score < 0 else 'Neutral.'}",  # noqa: E501
    ...
    async def _analyze_inflation(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        return FactorAnalysis(
            ...
            confidence=0.80,
            reasoning_zh=reasoning,
            reasoning_en=f"Inflation={inflation:.1f}%. {'High inflation supports gold.' if score > 0 else 'Low inflation weighs on gold.' if score < 0 else 'Neutral.'}",  # noqa: E501
    ...
    async def _analyze_geopolitical(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        return FactorAnalysis(
            ...
            confidence=0.70,
            reasoning_zh=reasoning,
            reasoning_en=f"Geopolitical risk={geo_score}. {'High risk boosts gold.' if score > 0 else 'Low risk reduces safe-haven demand.' if score < 0 else 'Neutral.'}",  # noqa: E501
    ...
    async def _analyze_central_bank(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        return FactorAnalysis(
            ...
            confidence=0.85,
            reasoning_zh=reasoning,
            reasoning_en=f"Fed rate={fed_rate:.2f}%. {'High rates weigh on gold.' if score < 0 else 'Low rates support gold.' if score > 0 else 'Neutral.'}",  # noqa: E501
    ...
    async def _analyze_gold_etf(self, context: dict[str, Any]) -> FactorAnalysis:
        ...
        return FactorAnalysis(
            ...
            confidence=0.80,
            reasoning_zh=reasoning,
            reasoning_en=f"ETF flow=${etf_flow / 1e6:.0f}M. {'Inflow supports gold.' if score > 0 else 'Outflow pressures gold.' if score < 0 else 'Neutral.'}",  # noqa: E501
    ...
    def _generate_report(
        self, date: str, current_price: float, factors: list[FactorAnalysis], composite_score: float
    ) -> dict[str, Any]:
        """生成基本面分析報告"""
        ...
        # 計算加權平均置信度
        avg_confidence = sum(f.confidence * f.weight for f in factors) / sum(
        ...
        for f in factors:
            direction_emoji = (
                "📈"
                if f.direction == FactorDirection.POSITIVE
                else "📉"
                if f.direction == FactorDirection.NEGATIVE
                else "➡️"
            )
    ...
    def sensitivity_analysis(self, factor_changes: dict[FactorType, float]) -> dict[str, Any]:
        """
