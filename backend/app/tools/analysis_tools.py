...
class AnalysisTools:
    ...
    async def calculate_rsi(
        self, data: list[float], period: int = 14, method: str = "wilders"
    ) -> list[float]:
        """
        ...
        if method == "wilders":
            ...
            for i in range(period, len(changes)):
                ...
                period_losses = [-loss for loss in changes[i - period : i] if loss < 0]
    ...
    async def find_support_resistance(
        self, data: list[float], window: int = 5
    ) -> dict[str, list[float]]:
        """
        識別支撐位和阻力位
    ...
    def _cluster_levels(self, levels: list[float], tolerance: float = 5.0) -> list[float]:
        """將接近的價格位聚類"""
        if not levels:
            return []
    ...
    async def analyze_trend(
        self, data: list[float], short_period: int = 10, long_period: int = 30
    ) -> dict[str, Any]:
        """
        分析價格趨勢
