...
class AlphaVantageAdapter(BaseDataSource):
    ...
    async def get_price(self, symbol: str) -> MarketData:
        ...
        if "Note" in data:
            raise RuntimeError(
                "Alpha Vantage 速率限制已觸發。免費版限制 5 calls/min。請稍後再試或升級至付費版。"
            )
        ...
        if data.get("Global Quote"):
            ...
            return MarketData(
                ...
                timestamp=datetime.fromisoformat(
                    quote.get(
                        "07. latest trading day", datetime.now(timezone.utc).date().isoformat()
                    )
                ),
                currency="USD",
                source=self.name,
    ...
    async def get_historical(
        ...
        """
        if not self.api_key or self.api_key == "":
            raise ValueError("Alpha Vantage API key 未設定。請在 .env 設定 ALPHA_VANTAGE_API_KEY。")
        ...
        if "Note" in data:
            raise RuntimeError("Alpha Vantage 速率限制已觸發。免費版限制 5 calls/min。")
