...
class DataQualityReport:
    ...
    def _calculate_score(
        self, total: int, valid: int, missing: int, duplicates: int, outliers: int
    ) -> float:
        """計算品質分數"""
        if total == 0:
            return 0.0
    ...
    def generate_report(
        ...
        logger.info(
            f"Data quality report generated: score={completeness_score:.1f}%, grade={report['summary']['quality_grade']}"  # noqa: E501
        )

        return report

    def _analyze_missing(self, data: list[dict[str, Any]], value_field: str) -> dict[str, Any]:
        """分析缺失值"""
        total = len(data)
    ...
    def _analyze_outliers(self, data: list[dict[str, Any]], value_field: str) -> dict[str, Any]:
        """分析異常值"""
        # 提取有效值
        valid_values = [item[value_field] for item in data if item.get(value_field) is not None]

        if len(valid_values) < 3:
            return {"outlier_count": 0, "outlier_rate": 0, "method": "insufficient_data"}
        ...
        return {
            ...
            "outlier_rate": round(
                max(zscore_outliers, iqr_outliers, marked_outliers) / len(data) * 100, 2
            ),
            "zscore_outliers": int(zscore_outliers),
            "iqr_outliers": int(iqr_outliers),
            "marked_outliers": marked_outliers,
            "bounds": {
                "zscore": {"lower": round(mean - 3 * std, 2), "upper": round(mean + 3 * std, 2)},
                "iqr": {"lower": round(lower, 2), "upper": round(upper, 2)},
            },
        }
    ...
    def _analyze_continuity(self, data: list[dict[str, Any]]) -> dict[str, Any]:
        """分析時間連續性"""
        # 提取有時間戳的記錄
        dated_items = [
            (item.get("timestamp"), item) for item in data if item.get("timestamp") is not None
        ]
        ...
        for i in range(1, len(timestamps)):
            ...
            if isinstance(t1, datetime) and isinstance(t2, datetime):
                ...
                # 如果間隔超過 24 小時，視為間斷
                if delta > 24:
                    gaps.append({"from": str(t1), "to": str(t2), "hours": round(delta, 2)})
    ...
    def _analyze_distribution(self, data: list[dict[str, Any]], value_field: str) -> dict[str, Any]:
        """分析數值分佈"""
        valid_values = [item[value_field] for item in data if item.get(value_field) is not None]
    ...
    def _calculate_completeness_score(
        self, total_records: int, missing_count: int, outlier_count: int, duplicate_count: int
    ) -> float:
        """計算完整性評分 (0-100)"""
        if total_records == 0:
            return 0
    ...
    def generate_summary(self, data: list[dict[str, Any]], value_field: str = "price") -> str:
        """
        生成人類可讀的品質摘要
