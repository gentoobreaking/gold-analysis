...
class DataCollectorAgent:
    ...
    def __init__(self, db: Database = None, retry_config: RetryConfig = None):
        ...
        self.adapters = {
            ...
            "YAHOO": YahooFinanceAdapter(timeout=self.retry_config.timeout, convert_to_twd=True),
        }
    ...
    def collect_all_prices(self, sources: list[str] = None) -> dict[str, CollectionResult]:
        ...
        with ThreadPoolExecutor(max_workers=len(sources)) as executor:
            # 提交所有任務
            future_to_source = {
                executor.submit(self._collect_from_source, source): source for source in sources
            }
            ...
            for future in as_completed(future_to_source):
                ...
                try:
                    ...
                    results[source] = CollectionResult(source=source, success=False, error=str(e))
    ...
    def _collect_from_source(self, source: str) -> CollectionResult:
        ...
        if not adapter:
            return CollectionResult(source=source, success=False, error=f"Unknown source: {source}")
        ...
        while retries <= self.retry_config.max_retries:
            try:
                ...
                return CollectionResult(
                    source=source, success=True, data=data, duration_ms=duration_ms, retries=retries
                )
                ...
                if retries <= self.retry_config.max_retries:
                    logger.warning(
                        f"{source} fetch failed (attempt {retries}), retrying in {delay}s: {e}"
                    )
                    time.sleep(delay)
        ...
        duration_ms = int((time.time() - start_time) * 1000)
        return CollectionResult(
            source=source, success=False, error=last_error, duration_ms=duration_ms, retries=retries
        )
    ...
    def _validate_single(self, item: dict) -> bool:
        ...
        has_price = any(
            item.get(field) is not None and item.get(field, 0) > 0 for field in price_fields
        )
        if not has_price:
            return False
