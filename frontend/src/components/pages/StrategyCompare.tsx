...
const StrategyCompare: React.FC = () => {
  ...
  const runCompare = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await runBacktestCompare(prices, [
        "ma_crossover",
        "rsi",
        "macd",
        "combined",
  ...
  const rows = result
    ? (Object.entries(result.results) as [string, StrategyRow][])
    : [];
  ...
  return (
    <div className="p-6 text-white">
      ...
      <p className="text-sm text-gray-400 mb-4">
        使用真實歷史收盤價（price_history），對 {prices.length}{" "}
        筆資料並排比較多策略績效（向量化回測引擎 T063）。
      </p>

      {error && (
        <div className="mb-4 px-3 py-2 rounded bg-red-900/50 text-red-300 text-sm">
          {error}
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800 text-gray-300">
                ...
                {METRIC_COLS.map((c) => (
                  <th key={c.key} className="text-right px-4 py-2">
                    {c.label}
                  </th>
                ))}
              </tr>
                ...
                >
                  ...
                  {METRIC_COLS.map((c) => {
                    ...
                    return (
                      <td
                        key={c.key}
                        className={`text-right px-4 py-2 ${color}`}
                      >
                        {text}
                      </td>
                    );
