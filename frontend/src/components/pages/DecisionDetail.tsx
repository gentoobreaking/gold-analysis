...
const SignalGauge: React.FC<{ decision: DecisionResponse }> = ({
  decision,
}) => {
  ...
  const gaugeColor =
    action === "buy" ? "#22c55e" : action === "sell" ? "#ef4444" : "#f59e0b";
  const bgTrack = "bg-slate-700";

  const labelText =
    action === "buy" ? "買入信號" : action === "sell" ? "賣出信號" : "持有觀望";
    ...
    <div className={`rounded-xl p-5 border ${borderColor} bg-slate-800`}>
      ...
      <div className="mt-4 pt-3 border-t border-slate-700 flex justify-between items-center">
        ...
        <div className="text-white font-semibold">
          ${decision.price.toLocaleString()} NT/克
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-8 text-gray-500 text-sm">
          暫無{tab === "buy" ? "買入" : tab === "sell" ? "賣出" : "持有"}信號
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map((s, i) => (
            ...
            >
              <div>
                <div
                  className={`text-sm font-semibold ${actionColor(s.action)}`}
                >
                  {s.signal ??
                    (s.action === "buy"
                      ? "買入"
...
const DecisionHistory: React.FC<{ history: HistoryRow[] }> = ({ history }) => {
  ...
  const actionBadge = (a?: string) => {
    if (a === "buy")
      return (
        <span className="px-2 py-0.5 rounded-full bg-green-900/50 text-green-400 text-xs font-medium">
          買入
        </span>
      );
    if (a === "sell")
      return (
        <span className="px-2 py-0.5 rounded-full bg-red-900/50 text-red-400 text-xs font-medium">
          賣出
        </span>
      );
    if (a === "hold")
      return (
        <span className="px-2 py-0.5 rounded-full bg-yellow-900/50 text-yellow-400 text-xs font-medium">
          持有
        </span>
      );
  ...
  const formatTs = (ts: string) => {
    try {
      ...
      return {
        ...
        time: d.toLocaleTimeString("zh-TW", {
          hour: "2-digit",
          minute: "2-digit",
        }),
      };
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-700">
          ...
          <div className="flex gap-1">
            <button
              disabled={page === 1}
              onClick={() => setPage(1)}
              className="px-2 py-1 bg-slate-700 disabled:opacity-30 rounded text-xs"
            >
              ⏮
            </button>
            <button
            ...
            <span className="px-3 py-1 bg-slate-600 rounded text-xs text-white">
              {page}
            </span>
            <button
...
const DecisionDetail: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const enriched: HistoryRow[] = histData.data
        .map((h, i) => {
          const pct = Math.random();
          const action = (
            pct > 0.6 ? "buy" : pct > 0.3 ? "hold" : "sell"
          ) as HistoryRow["action"];
          return {
            ...h,
            action,
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {error && (
        <div className="bg-red-900/30 text-red-400 p-4 rounded-lg text-sm">
          ⚠️ {error}
        </div>
      )}
...
const ExplanationCard: React.FC<{ explanation?: DecisionExplanation }> = ({
  explanation,
}) => {
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
              ...
              return (
                <div key={i}>
                  ...
                  <div className="h-1.5 rounded-full bg-slate-700 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${dirColor}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
      ...
      {explanation.triggered_rules &&
        explanation.triggered_rules.length > 0 && (
