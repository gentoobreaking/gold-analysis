...
const FactorBars: React.FC<{ exposure: Record<string, number> }> = ({ exposure }) => {
...
const Card: React.FC<{ title: string; children: React.ReactNode }> = ({
  ...
  >
    <h3 style={{ margin: '0 0 12px', color: '#e2e8f0', fontSize: 15 }}>{title}</h3>
    {children}
  </div>
);
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      ...
      <p style={{ color: '#94a3b8', marginTop: 0 }}>
        跨資產相關性熱圖與因子曝險。GOLD 採真實歷史收盤價；
        DXY / 實質利率 / BTC / SPX 為示範用合成因子。
      </p>
