...
"""
    ...
    try:
        result = subprocess.run(
            ["node", script_path], capture_output=True, text=True, timeout=30, cwd=tmp_dir
        )
...
def generate_price_chart(history, sell_price, buy_price, current_time):
    ...
    if prev_trading_day in history["daily"]:
        prev_data = history["daily"][prev_trading_day]
    ...
    if today in history["intraday"]:
        for point in history["intraday"][today]:
            ...
            dates.append(dt)
            sell_prices.append(point.get("sell"))
            buy_prices.append(point.get("buy"))
...
def main():
    ...
    parser.add_argument(
        "--add-target", type=str, help="新增價格點位 (格式: price:type:label，如 4500:buy:進場點)"
    )
    ...
    current_buy = price_data.get("buy_price") or current_sell  # 如果沒有買入價就用賣出價
    ...
    if args.daily:
        ...
        prev_data = history["daily"].get(prev_trading_day)

        # 生成走勢圖
        chart_path = generate_price_chart(history, current_sell, current_buy, current_time)

        msg = format_daily_report(current_sell, current_buy, current_time, prev_data, is_monday)
        ...
        print(f"✅ 已發送每日報告：賣出 {current_sell:,} / 買入 {current_buy:,} 元")
        return
    ...
    if args.check:
        ...
        last_buy = state.get("last_buy")

        if last_sell is None:
            # 首次運行，只記錄價格
            state["last_sell"] = current_sell
            state["last_buy"] = current_buy
            state["last_time"] = current_time
            save_state(state)
            print(f"✅ 首次運行，已記錄價格：賣出 {current_sell:,} / 買入 {current_buy:,} 元")
            return

        price_change = abs(current_sell - last_sell)
        threshold = config["threshold"]

        if price_change >= threshold:
            # 價格變動超過閾值，發送通知
            msg = format_price_change(last_sell, current_sell, last_buy, current_buy, threshold)
            send_telegram_message(msg)
        ...
        save_state(state)
        print(
            f"📊 目前價格：賣出 {current_sell:,} / 買入 {current_buy:,} 元（變動：{price_change} 元）"
        )
